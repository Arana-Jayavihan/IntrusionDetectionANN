...
def addToElastic(data):
    count = 0
    created = 0
    failed = 0
    insertUrl = "http://localhost:64298/tshark/_doc/?pretty"
    headers = {'Content-Type': 'application/json'}
    for obj in data:
        count+=1
        tmpObj = findFieldValues(obj, fields)
        response = requests.post(insertUrl, json=tmpObj.copy(), headers=headers).json()
        print(response)
        if response['result'] != 'created':
            failed+=1
        else:
            created+=1
    return created, failed, count

with open('tshark.json', 'r') as file:
    data = json.load(file)
    created, failed, count = addToElastic(data)
    print(str(count) + " Packets in total")
    print(str(created) + " Packets added to elastic")
    print(str(failed) + " Packets failed")